...
class UnifiedLLMClient:
    ...
    def __init__(self, cfg: dict[str, Any], profile: str = "graph", debug_logger=None):
        ...
        if profile_key not in models_cfg:
            raise ValueError(
                f"Model profile '{profile}' not found in config and no fallback available"
            )
        ...
        try:
            ...
            env_verbose = _os.environ.get("HOUND_LLM_VERBOSE", "").lower() in {
                "1",
                "true",
                "yes",
                "on",
        ...
        if provider_name == "openai":
            ...
            self.provider = XAIProvider(**common_kwargs, verbose=llm_verbose)
        elif provider_name == "deepseek":
            self.provider = DeepSeekProvider(**common_kwargs, verbose=llm_verbose)
        elif provider_name == "openrouter":
            self.provider = OpenRouterProvider(**common_kwargs, verbose=llm_verbose)
        elif provider_name == "groq":
            self.provider = GroqProvider(**common_kwargs, verbose=llm_verbose)
        elif provider_name == "mock":
            # Mock provider for testing
            mock_instance = model_config.get("mock_instance")
            self.provider = MockProvider(**common_kwargs, mock_instance=mock_instance)
        else:
        ...
        if llm_verbose:
            print(
                f"[*] Initialized {self.provider.provider_name} provider with model: {self.model}"
            )
    ...
    def parse(
        self, *, system: str, user: str, schema: type[T], reasoning_effort: str | None = None
    ) -> T:
        """
        ...
        try:
            try:
                response = self.provider.parse(
                    system=system, user=user, schema=schema, reasoning_effort=reasoning_effort
                )
            except TypeError:
            ...
            if self.debug_logger:
                ...
                self.debug_logger.log_interaction(
                    ...
                    user_prompt=user,
                    response=(
                        response.dict() if response and hasattr(response, "dict") else response
                    ),
                    schema=schema,
                    duration=duration,
                    error=error,
